...
//campos de la bd "NUMCONSTANCIA", "SEXO", 
//"NOMBREALUMNO", "NUMCONTROL", "CARRERA", 
//"DEPENDENCIA", "ACTIVIDADES", "PERIODO", 
//"INTERESADO", "HORAS"
	...
	if(isEmpty(options)){		
		...
		if(options.carrera!==undefined&&options.carrera instanceof(Array)){		
			for(i=0;i<options.carrera.length;i++)
				carreras.push({CARRERA:{$like:aux(options.carrera[i])}})
		}else carreras[0]={CARRERA:{$like:aux(options.carrera)}};

		servidor.findAll({where:{
								SEXO:{$like:aux(options.sexo)},
								FECHAINICIO:{$between:rangoFecha(options)},
								DEPENDENCIA:{$like:aux(options.dependencia.toUpperCase())},
								$or:carreras
								}})	
		.then(function(data){			
			 res.status(200)
...
var servidorNumControl=function(req,res,next){
		...
		servidor.findAll({where:{$or:[
									{NUMCONTROL:{$like:nc+'%'}},
									{NOMBREALUMNO:{$like:nc.toUpperCase()+'%'}}
								]}})
		.then(function(data){			
			if(data.length>0){
			res.status(200)
			.json({status:'succes',data:data,message:"Estudiante encontrado"});						
			next();}
			else{
			res.status(404)
			.json({status:'failled',message:"Estudiante no encontrado"});
			next();
			}	
		});			
}
...
var updateServidor=function(req,res,next){	
	...
	servidor.findOne({where:{id:req.params.id}})
		.then(function(data){
			if(data){
		  		var sexo,interesado;			  	
		  		if(req.body.sexo=="el"){
			...
			servidor.update(servidorParams,{
					...
					res.status(200)
					.json({status:'Success',data:d[1],message:"Registros actualizados"});									
					next();
				}).error(function(e){
					console.log(e);
					next();
...
var buscarID=function(req,res,next){
	servidor.findOne({where:{id:req.params.id}})
		.then(function(d){
			if(d)
				res.status(200)
					.json({status:"Success",data:d,message:"Encontrado"});
			else
				res.status(404)
					.json({status:"not found",message:"No encontrado"});		
		})
		.error(function(e){
			console.log(e)
			res.status(500);
...
var constancias=function(req,res,next){
	...
	servidor.findAll({where:{
			FECHAINICIO:{$between:rangoFecha(req.query)}
	}})
	.then(function(d){
		if(d.length>0){
			crearConstancia(d);
			...
			res.status(404)
				res.json({status:"not found",message:"No hay servidores que concluyeran servicio social en ese periodo"})
		}
		})
		.error(function(e){
			res.status(500);
