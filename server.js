...
var validar=function(req,res,next){
	if(req.method=="POST"||req.method=="PUT"){
		var body=req.body;
		if(body.numControl!=null&&body.numControl!==""&&body.nombre!==""&&body.nombre!==null
			&&body.fechainicio!==null&&body.fechatermino!==null){
			console.log("paso")
			next();	}
		else
			res.status(400).
			json({status:"Denegado",message:"Operacion denegada por datos incorrectos"});	
	}else{
		next();
	}
};
...
var existe=function existe(req,res,next){	
	if(req.method=="POST"){	
	servidor.findOne({where:{NUMCONTROL:req.body.numControl}})
		.then(function(d){		
			if(d){ 				
				res.status(400)
					.json({message:"Ese Número de control ya existe",data:d});
			}
			else{
